...
public:
	...
	*
	* Throws:
	*     IND_RANG - this will throw an index out of range error if we try to add an element that is 
	*                out of range of our linked list. 
	*/
	...
	*
	* Throws:
	*	 NOT_FOUND - this will output the not found string if the element does not exist
	*
	*/
	...
	*
	*/
	void remove(string element) throw (string); //removes the element u wanna remove at a particular index
	...
	*
	* Throws:
	*	 LIST_EMPTY - this will throw the list empty string if we try to remove an element from the list
	*                 but there is nothing in the list because it is empty.
	*/
